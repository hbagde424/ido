$(document).ready(function() {
        ...
        if ($('#types_of_service_id').length && $('#types_of_service_id').val()) {
         ...
         $.ajax({
                   ...
                   data:   {
                    ...
                    q:'', // search term
                    location_id: $("#select_location_id").val(), // search term
                  
            },
                    success: function(response) {
                    // Handle the response here
                    var customersDropdown = $('#transactionSelect');
                    customersDropdown.empty(); // Clear existing options
                    customersDropdown.append('<option value="" disabled selected>Select a customer</option>'); // Add placeholder
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        __write_number(tr.find('input.pos_line_total'), line_total, false, 2);
                ...
                var pos_unit_commision_inc_tax = __read_number(tr.find('input.pos_unit_commision_inc_tax'));

        __write_number(tr.find('input.pos_unit_total_commision_inc_tax'), pos_unit_commision_inc_tax*entered_qty, false, 2);
    ...
    $('table#pos_table tbody tr').each(function() {
        price_total_c = price_total_c + __read_number($(this).find('input.pos_unit_total_commision_inc_tax'));
    });
    $('#sale_commision').val(__currency_trans_from_en(price_total_c, false));
   ...
   //If change in unit price update price including tax and line total
    $('table#pos_table tbody').on('change', 'input.pos_unit_commision_inc_tax', function() {
        ...
        __write_number(tr.find('input.pos_unit_total_commision_inc_tax'), line_total);
...
function pos_discount(total_amount) {
    ...
    var discount = __calculate_amount(calculation_type, calculation_amount, total_amount);
        // alert("sheetal");
    $('span#total_discount').text(__currency_trans_from_en(discount, false));
    $("#total_discountnewdata").val(__currency_trans_from_en(discount, false)); // Discount ko total_discountnewdata mein show karna
