    ...
    if ($('#sr_date_filter').length == 1) {
        ...
        sr_sales_commission_report = $('table#sr_sales_with_commission_table').DataTable({
            ...
            columns: [
                ...
                { data: 'sale_commision', name: 'sale_commision' },
                { data: 'commision_status', name: 'commision_status' },
            ...
            columnDefs: [
                {
                    ...
                    targets: [6],
                            ...
                            "footerCallback": function ( row, data, start, end, display ) {
                                ...
                                var paid_total_commision = 0;
                                var unpaid_total_commision = 0;
                          

                                for (var r in data){
                            if(data[r].sale_commision != null)
                            {
                                total_commision += data[r].sale_commision ;
                                
                                if(data[r].commision_status=='Paid'){
                                    paid_total_commision += data[r].sale_commision ;
                                }
                                   
                                if(data[r].commision_status == "Unpaid"){
                                    unpaid_total_commision += data[r].sale_commision ;
                                }
                            }
                            $("#sr_total_commision").html(total_commision);
                            $("#sr_total_paid_commision").html(paid_total_commision);
                            $("#sr_total_unpaid_commision").html(unpaid_total_commision);
...
fnDrawCallback: function(oSettings) {
    ...
    sell_payment_report = $('table#sell_payment_report_table').DataTable({
        ...
        columns: [
          ...
          "columnDefs": [{
            "targets": [1], // The index of the column where you want to merge the data
            "render": function(data, type, row, meta) {
                return row.customer + ',  ' + row.customer_address; // Concatenating the values
            }
        }],
