$(document).ready(function() {
    ...
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '/reports/customer-salse-reportd',
            data: function(d) {
        ...
        fnDrawCallback: function(oSettings) {
            ...
            // Initialize an empty array to store the latest 10 total_invoice values
            var latestTotalInvoices = [];
            var latestTotalcustomer = [];
            
            // Get the DataTable instance
            var table = $('#supplier_report_tbl').DataTable();
            
            // Get the data from the DataTable
            var data = table.rows().data().toArray();
            
            // Sort the data based on the 'name' column in descending order to get the latest invoices
            // data.sort(function(a, b) {
            //     return new Date(b.name) - new Date(a.name);
            // });
            
            // Extract the latest 10 total_invoice values
            for (var i = 0; i < Math.min(data.length, 10); i++) {
                
                // Get the text content of the span element
            var spanText =data[i].total_invoice;
            var textContent = $(spanText).text();
            
            // Regular expression to extract numerical value
            var numericalValue = textContent.match(/\d+(\,\d+)*\.\d+/g);
            
            // If numericalValue is an array, get the first element
            var valueOnly = numericalValue ? numericalValue[0] : null;
            
            
                latestTotalInvoices.push(valueOnly);
                
                 // Get the text content of the span element
            var html =data[i].name;
            var tempElementt = document.createElement('div');
            tempElementt.innerHTML = html;
            
            // Extract the text content (which includes "Ndiambour Carreaux")
            var textContentq = tempElementt.textContent.trim();
            
            // Extract "Ndiambour Carreaux" from the text content
            //var extractedText = textContentq.split(', ')[1];
             
             console.log(html);
            
            
                latestTotalcustomer.push(textContentq)
            }
            
            console.log(latestTotalcustomer);
            
            var data = latestTotalInvoices.map(function(value) {
...
// Check if there's already a chart instance associated with the canvas
if (canvas) {
    // Retrieve the context of the canvas
    var ctx = canvas.getContext('2d');

    // Retrieve the existing chart instance if any
...
var myChart = new Chart(ctx, {
                type: 'pie',
                data: {
                    labels: latestTotalcustomer, // Array of strings
                    datasets: [{
                        data: data, // Array of numerical values
                        backgroundColor: [
