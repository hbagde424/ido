        ...
        if (! empty($product) && $product->enable_stock == 1) {
            ...
            foreach ($transactions as $transaction) {
                foreach ($transaction->purchase_lines as $purchase_line) {
                    ...
                    $purchase_lines[$purchase_line->variation_id][$k]['profit_loss_p'] = $transaction->profit_loss_p;
                }
            }
                    ...
                    if (array_key_exists($location_id, $locations)) {
                        foreach ($value as $vid => $purchase_lines_data) {
                            ...
                            foreach ($purchase_lines_data as $k => $pl) {
                                ...
                                $profit_loss_p = ! empty($pl['profit_loss_p']) ? $pl['profit_loss_p'] : null;
                        ...
                        if (! empty($edit_purchase_lines)) {
                            foreach ($edit_purchase_lines as $t_id => $purchase_lines) {
                                ...
                                $transaction->profit_loss_p = $edit_transaction_data[$transaction->id]['profit_loss_p'];
                                $transaction->update();
