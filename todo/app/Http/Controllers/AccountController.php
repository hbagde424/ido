...
class AccountController extends Controller
{
    ...
    public function __construct(CashRegisterUtil $cashRegisterUtil, Util $commonUtil, ModuleUtil $moduleUtil)
    {
    ...
    public function create()
    {
        ...
        return view('account.create')
                ->with(compact('account_types','payment_types'));
    }
        ...
        if (request()->ajax()) {
            ...
            ->with(['transaction', 'transaction.contact', 'transfer_transaction', 'transaction.transaction_for'])
                            ...
                            ->select(['account_transactions.type', 'account_transactions.amount', 'operation_date',
                                'account_transactions.sub_type', 'transfer_transaction_id','payment_method',
                                'A.id as account_id',
                                'account_transactions.transaction_id',
                                'account_transactions.id',
                                'account_transactions.note as tnote',
                                'tp.is_advance',
                                'tp.is_return',
                                'tp.payment_ref_no',
                                'tp.note',
            ...
            if (! empty(request()->input('pmethod'))) {
            $accounts->where('tp.method', request()->input('pmethod'));
            }
...
$paymtnetotal =AccountTransaction::join('accounts as A', 'account_transactions.account_id', '=', 'A.id')
    ->leftJoin('transaction_payments as TP', 'account_transactions.transaction_payment_id', '=', 'TP.id')
    ...
    ->groupBy('account_transactions.payment_method')
    ->orderBy('TP.method', 'ASC')
    ->select([
        'account_transactions.payment_method as method',
        DB::raw("SUM(CASE WHEN account_transactions.type = 'credit' THEN account_transactions.amount ELSE -account_transactions.amount END) AS net_amount")
    ])
    ->get();
        ...
        return view('account.show')->with(compact('account','payment_types','paymtnetotal'));
    }
 ...
 $payment_types = $this->commonUtil->payment_types(null, true, $business_id);
            ...
            return view('account.transfer')
                ->with(compact('from_account', 'to_accounts','payment_types'));
        }
    }
 ...
 $payment_types = $this->commonUtil->payment_types(null, true, $business_id);
            ...
            return view('account.deposit')
                ->with(compact('account', 'account', 'from_accounts','payment_types'));
        }
    }
    ...
    {   
        ...
        $account = Account::leftjoin(
            ...
            ->select('accounts.*', DB::raw("SUM( IF(AT.type='credit', amount, -1 * amount) ) as balance"),
                DB::raw("SUM( IF(AT.type='credit' AND AT.payment_method='Cash',  amount, 0) ) as total_credit_cash"),
                DB::raw("SUM( IF(AT.type='debit' AND AT.payment_method='Cash', amount, 0) ) as total_debit_cash")
   )
            ->first();
    ...
    public function __getPaymentDetails($row)
    {
        ...
        if (! empty($row->sub_type)) {
            $details = __('account.'.$row->sub_type).'<br>'.$row->tnote;
            ...
            if (! empty($row->transaction->type)) {
                if ($row->transaction->type == 'purchase')  {
                    $details = __('lang_v1.purchase').'<br><b>'.__('').':</b><b>'.
                    __('').'</b> <a href="#" data-href="'.action([\App\Http\Controllers\PurchaseController::class, 'show'], [$row->transaction->id]).'" class="btn-modal" data-container=".view_modal">'.$row->transaction->ref_no.'</a>';
                } elseif ($row->transaction->type == 'expense') {
                    $details = __('').'<br><b>'.__('').'</b>'.'<br>'.$row->transaction->additional_notes;
                } elseif ($row->transaction->type == 'sell') {
                    $is_return = $row->is_return == 1 ? ' ('.__('lang_v1.change_return').')' : '';
                    $details = __('sale.sale').$is_return.'<br><b>'.__('').':</b> <b>'.
                    __('').'</b> <a href="#" data-href="'.action([\App\Http\Controllers\SellController::class, 'show'], [$row->transaction->id]).'" class="btn-modal" data-container=".view_modal">'.$row->transaction->invoice_no.'</a>';
                
                        } elseif ($row->transaction->type == 'sell_return') {
                    $details = __('Sell Return Against').'<br>'.$row->transaction->id;
    ...
    public function editAccountTransaction($id)
    {
        ...
        return view('account.edit_account_transaction')
            ->with(compact('accounts', 'account_transaction','payment_types'));
    }
    ...
    public function updateAccountTransaction(Request $request, $id)
    {
        ...
        try {
            ...
            $account_transaction->payment_method = $request->input('payment_method');
            ...
            $transfer_transaction->payment_method = $request->input('payment_method');
