...
$start_date = Carbon::createFromDate($currentYear, 1, 1)->toDateString();

// Last date of the current year
$end_date = Carbon::createFromDate($currentYear, 12, 31)->toDateString();

    $permitted_locations = auth()->user()->permitted_locations();
    $dataaa = $this->transactionUtil->getProfitLossDetails($business_id, $location_id=null, $start_date, $end_date, $user_id==null, $permitted_locations);
       
 //  dd($dataaa);

$data = [   
            'labels' => ['Expenses', 'Income'],
            'data' => [2544, $dataaa['total_expense']],
        ];
        ...
        // Fetch Attendance for current user for today
        $attendance = null;
        if ($this->moduleUtil->isModuleInstalled('Essentials')) {
            $attendance = \Modules\Essentials\Entities\EssentialsAttendance::where('user_id', auth()->user()->id)
                ->whereDate('clock_in_time', \Carbon\Carbon::today())
                ->first();
        }

        // Fetch Birthdays
        $birthdays = User::where('business_id', $business_id)
            ->whereMonth('dob', \Carbon\Carbon::now()->month)
            ->whereDay('dob', \Carbon\Carbon::now()->day)
            ->get();

         return view('home.index', compact('data' ,'data2','sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin', 'attendance', 'birthdays'));
    }
    ...
    public function getTotals()
    {
        if (request()->ajax()) {
            ...
            $sell_detailsnew = $this->transactionUtil->getSellTotals($business_id, date('Y-m-d'), date('Y-m-d'), $location_id);
            $total_sell_inc_taxnew = ! empty($sell_detailsnew['total_sell_inc_tax']) ? $sell_detailsnew['total_sell_inc_tax'] : 0;



            $output['today_sell'] = $total_sell_inc_taxnew;
            ...
            $firstDayFormatted = $firstDayOfMonth->toDateString();
            $lastDayFormatted = $lastDayOfMonth->toDateString();
            
            $output['f1']=$firstDayFormatted;
            $output['f2']=$lastDayFormatted;
  

              $sell_detailsnew1 = $this->transactionUtil->getSellTotals($business_id, $firstDayFormatted, $lastDayFormatted, $location_id);
            $total_sell_inc_taxnew1 = ! empty($sell_detailsnew1['total_sell_inc_tax']) ? $sell_detailsnew1['total_sell_inc_tax'] : 0;



            $output['monthly_sell'] = $total_sell_inc_taxnew1;
            ...
            $output['net'] = $output['total_sell'] - $output['total_purchase'] - $output['total_expense'];
            
         
                $data = $this->transactionUtil->getProfitLossDetails($business_id, $location_id, $start, $end,  $user_id = null, $permitted_locations);
                $output['data'] = $data;

    $output['closing_stock_by_sp'] = $this->transactionUtil->getOpeningClosingStockhome(
            $business_id,
            $end,
            $location_id,
            false,
    ...
    {
        \Log::info('loadMoreNotifications called');
        ...
        \Log::info('Parsed notifications data count: ' . count($notifications_data));
    ...
    return view('home.pendingShipments', compact('sells_chart_1', 'sells_chart_2', 'all_locations'));
    ...
    public function productStockAlert(){
        ...
        return view('home.productStockAlert', compact('sells_chart_1', 'sells_chart_2', 'all_locations'));
    }
