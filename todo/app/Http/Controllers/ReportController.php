...
public function getCustomer(Request $request)
    {
        ...
        return view('report.contactreport')
        ->with(compact('customer_group', 'types', 'business_locations', 'contact_dropdown'));
    }
public function getCustomerd(Request $request)
    {
        if (! auth()->user()->can('contacts_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('report.contactreportd')
        ->with(compact('customer_group', 'types', 'business_locations', 'contact_dropdown'));
    }
    /**
        ...
        if ($request->ajax()) {
            ...
            $datatable = Datatables::of($products)
                ...
                ->setRowClass(function ($row) {
                    return $row->enable_stock && $row->stock <= $row->alert_quantity ? 'bg-danger error' : '';
                })
...
PurchaseLine::where('id', $id)->update(['postatus' => $postatus]);
    ...
    return redirect()->back()->with(['status' => [
     ...
     public function getStockReportproduct(Request $request)
    {
        if (! auth()->user()->can('stock_reportproduct.view')) {
            abort(403, 'Unauthorized action.');
        }
...
//ashok
 $results = Product::selectRaw('SUM(vld.qty_available) AS stock,products.name AS product,products.image AS image, products.id,products.type,
 products.trans_status, products.alert_quantity, vld.location_id,vld.variation_id')
...
// Check if location_id is provided in the request
if ($request->has('location_id')) {
    $location_id = $request->input('location_id');
    $results->where('vld.location_id', $location_id);
}

$results = $results->orderBy('products.id', 'asc')->get();

foreach($results as $key => $eachpro)
{
    
    $purchaseLines = PurchaseLine::select('purchase_lines.postatus', 'purchase_lines.product_id', 'transactions.location_id',
    DB::raw('SUM(purchase_lines.po_quantity_purchased) as po_quantity_purchased'),
    ...
    ->where('transactions.location_id', $eachpro->location_id)
    ->where('purchase_lines.product_id', $eachpro->id)
    ...
    $results[$key]->postatus  = $purchaseLines->postatus; 
    if($purchaseLines->quantity-$purchaseLines->po_quantity_purchased >0 ){
    $results[$key]->remaing_qty  = $purchaseLines->quantity-$purchaseLines->po_quantity_purchased; 
    }else{
          $results[$key]->remaing_qty  = ''; 
    }
}
$selectedlocaion='';
if( $request->input('location_id'))
{
    $selectedlocaion=$request->input('location_id');
}
        $categories = Category::forDropdown($business_id, 'product');
        $brands = Brands::forDropdown($business_id);
        $units = Unit::where('business_id', $business_id)->pluck('short_name', 'id');
        $business_locations = BusinessLocation::forDropdown($business_id, true);
        return view('report.stock_report_product')->with(compact('location_id','selectedlocaion','results','categories', 'brands', 'units', 'business_locations', 'show_manufacturing_data'));
    }
    
    
     public function getStockReportproductpdf(Request $request)
    {
        if (! auth()->user()->can('stock_reportproduct.view')) {
            abort(403, 'Unauthorized action.');
        }
...
if( $request->input('location_id'))
{
    $selectedlocaion=$request->input('location_id');
        ...
        $units = Unit::where('business_id', $business_id)->pluck('short_name', 'id');
        $business_locations = BusinessLocation::forDropdown($business_id, true);
         $html = view('report.stockreportpdf')->with(compact('location_id','selectedlocaion','results','categories', 'brands', 'units', 'business_locations', 'show_manufacturing_data'))->render();
            $mpdf = new \Mpdf\Mpdf([
    'curl' => true,
...
$data = [   
        ...
        return view('report.trending_products')
                    ->with(compact('data','products','chart', 'categories', 'brands', 'units', 'business_locations'));
    }
    
    
      public function getDeadProducts(Request $request)
    {
        if (! auth()->user()->can('dead_product_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $products = $this->productUtil->getDeadProducts($business_id, $filters);
...
$data = [   
        ...
        return view('report.dead_products')
                    ->with(compact('data','products','chart', 'categories', 'brands', 'units', 'business_locations'));
    }
                ...
                ->select(
                    DB::raw("IF(transaction_payments.transaction_id IS NULL, 
                              ...
                              DB::raw("IF(transaction_payments.transaction_id IS NULL, 
                                ...
                                (SELECT address_line_1 FROM transactions as ts JOIN
                                    contacts as c ON ts.contact_id=c.id
                                    WHERE ts.id=t.id 
                                )
                            ) as customer_address"),
    ...
    public function getProfit($by = null)
    {
        ...
        $query->select(DB::raw('SUM(IF (TSPL.id IS NULL AND P.type="combo", ( 
            SELECT Sum((tspl2.quantity - tspl2.qty_returned) * (tsl.unit_price_inc_tax - pl2.purchase_price_inc_tax)) AS total
                ...
                (TSPL.quantity - TSPL.qty_returned) * (transaction_sell_lines.unit_price_inc_tax - PL.purchase_price_inc_tax)) )) AS gross_profit , sale.transaction_date ,transaction_sell_lines.quantity')
           );
        ...
        if (! empty(request()->start_date) && ! empty(request()->end_date)) {
            ...
            // ->leftJoin('purchase_lines as PR', 'TSPL.purchase_line_id', '=', 'PR.id')
            ->addSelect(
                DB::raw("IF(P.type='variable', CONCAT(P.name, ' - ', PV.name, ' - ', V.name, ' ', ''), CONCAT(P.name, ' ',  '')) as product,
                V.product_id,
                PL.purchase_price_inc_tax as dpp_inc_tax,
                transaction_sell_lines.unit_price_inc_tax,
            ...
            ->addSelect('CU.name as customer', 'CU.supplier_business_name');
              ...
              if (! empty(request()->location_id)) {
         //   $query->where('PR.location_id', request()->location_id);
                }
