...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'sale_commision' => $this->num_uf($input['sale_commision']),
            'commision_status' => $input['commision_status'],
            ...
            'actual_discount' => $uf_data ? $this->num_uf($input['actual_discount']) : $input['actual_discount'],
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                                ...
                                if (! empty($product['modifier_price'][$key])) {
                                    ...
                                    $modifiers_formatted[] = new TransactionSellLine([
                                        'product_id' => $product['modifier_set_id'][$key],
                                         'pcommission' => $this->num_uf($product['unit_commision_inc_tax'][$key]),
                                        'commission_subtotal' => $this->num_uf($product['unit_total_commision_inc_tax'][$key]),
                                        'variation_id' => $value,
                ...
                $line = [
                    ...
                    'pcommission' => $this->num_uf($product['unit_commision_inc_tax']),
                    'commission_subtotal' => $this->num_uf($product['unit_total_commision_inc_tax']),
        ...
        foreach ($combo_items as $value) {
            ...
            $combo_lines[] = new TransactionSellLine([
                ...
                'variation_id' => $value['variation_id'],
                  'pcommission' => $this->num_uf($value['unit_commision_inc_tax']),
            'commission_subtotal' => $this->num_uf($value['unit_total_commision_inc_tax']),
    ...
    public function getReceiptDetailscopy($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        $il = $invoice_layout;

   $ids = [957, 956, 955, 954,953,952,951,950];
$transactions = Transaction::whereNotNull('invoice_no')->whereIn('invoice_no', $ids)->where('type','sell')
        ...
        $output = [ 
            ...
            $transaction_type = $transaction->type;
            $output[$key]['header_text']=isset($il->header_text) ? $il->header_text : '';
            $output[$key]['business_name']= ($il->show_business_name == 1) ? $business_details->name : '';
            $output[$key]['location_name'  ]= ($il->show_location_name == 1) ? $location_details->name : '';
            $output[$key]['sub_heading_line1']=trim($il->sub_heading_line1);
            $output[$key]['sub_heading_line2']=trim($il->sub_heading_line2);
            $output[$key]['sub_heading_line3']=trim($il->sub_heading_line3);
            $output[$key]['sub_heading_line4']=trim($il->sub_heading_line4);
            $output[$key]['sub_heading_line5']=trim($il->sub_heading_line5);
            $output[$key]['table_product_label']=$il->table_product_label;
            $output[$key]['table_qty_label']=$il->table_qty_label;
            $output[$key]['table_unit_price_label']=$il->table_unit_price_label;
            $output[$key]['table_subtotal_label']=$il->table_subtotal_label;
            //  $output[$key]['custom_field_1']=$transaction->custom_field_1;
            //  $output[$key]['custom_field_2']=$transaction->custom_field_2;
            //  $output[$key]['custom_field_3']=$transaction->custom_field_3;
            //  $output[$key]['custom_field_4']=$transaction->custom_field_4;
             $output[$key]['prefer_payment_method']=$transaction->prefer_payment_method;
             $output[$key]['pay_term_number']=$transaction->pay_term_number;
             $output[$key]['pay_term_type']=$transaction->pay_term_type;
             $output[$key]['additional_notes']=$transaction->additional_notes;
           

//Display name
$output[$key]['display_name'] = $output[$key]['business_name'];
if (! empty($output[$key]['location_name'])) {
   if (! empty($output[$key]['display_name'])) {
       $output[$key]['display_name'] .= ', ';
   }
   $output[$key]['display_name'] .= $output[$key]['location_name'];
}

//Codes
if (! empty($business_details->code_label_1) && ! empty($business_details->code_1)) {
   $output[$key]['code_label_1'] = $business_details->code_label_1;
   $output[$key]['code_1'] = $business_details->code_1;
}

if (! empty($business_details->code_label_1) && ! empty($business_details->code_1)) {
   $output[$key]['code_label_2'] = $business_details->code_label_2;
   $output[$key]['code_2'] = $business_details->code_2;
}

if ($il->show_letter_head == 1) {
   $output[$key]['letter_head'] = ! empty($il->letter_head) &&
   file_exists(public_path('uploads/invoice_logos/'.$il->letter_head)) ?
   asset('uploads/invoice_logos/'.$il->letter_head) : null;
}

//Logo
$output[$key]['logo'] = $il->show_logo != 0 && ! empty($il->logo) && file_exists(public_path('uploads/invoice_logos/'.$il->logo)) ? asset('uploads/invoice_logos/'.$il->logo) : false;
...
if (! empty($temp)) {
   $output[$key]['address'] .= implode(', ', $temp);
}

$output[$key]['website'] = $location_details->website;
$output[$key]['location_custom_fields'] = '';
$temp = [];
$location_custom_field_settings = ! empty($il->location_custom_fields) ? $il->location_custom_fields : [];
if (! empty($location_details->custom_field1) && in_array('custom_field1', $location_custom_field_settings)) {
   $temp[] = $location_details->custom_field1;
...
if (! empty($temp)) {
   $output[$key]['location_custom_fields'] .= implode(', ', $temp);
}

//Tax Info
if ($il->show_tax_1 == 1 && ! empty($business_details->tax_number_1)) {
   $output[$key]['tax_label1'] = ! empty($business_details->tax_label_1) ? $business_details->tax_label_1.': ' : '';

   $output[$key]['tax_info1'] = $business_details->tax_number_1;
}
if ($il->show_tax_2 == 1 && ! empty($business_details->tax_number_2)) {
   if (! empty($output[$key]['tax_info1'])) {
       $output[$key]['tax_info1'] .= ', ';
   }

   $output[$key]['tax_label2'] = ! empty($business_details->tax_label_2) ? $business_details->tax_label_2.': ' : '';

   $output[$key]['tax_info2'] = $business_details->tax_number_2;
}

//Shop Contact Info
$output[$key]['contact'] = '';
if ($il->show_mobile_number == 1 && ! empty($location_details->mobile)) {
   $output[$key]['contact'] .= '<b>'.__('contact.mobile').':</b> '.$location_details->mobile;
}
if ($il->show_alternate_number == 1 && ! empty($location_details->alternate_number)) {
   if (empty($output[$key]['contact'])) {
       $output[$key]['contact'] .= __('contact.mobile').': '.$location_details->alternate_number;
   } else {
       $output[$key]['contact'] .= ', '.$location_details->alternate_number;
   }
}
if ($il->show_email == 1 && ! empty($location_details->email)) {
   if (! empty($output[$key]['contact'])) {
   ...
   $output[$key]['contact'] .= '<br>'.__('business.email').': '.$location_details->email;
}

//Customer show_customer
$customer = Contact::find($transaction->contact_id);

$output[$key]['customer_info'] = '';
$output[$key]['customer_tax_number'] = '';
$output[$key]['customer_tax_label'] = '';
$output[$key]['customer_custom_fields'] = '';
if ($il->show_customer == 1) {
   $output[$key]['customer_label'] = ! empty($il->customer_label) ? $il->customer_label : '';
   $output[$key]['customer_name'] = ! empty($customer->name) ? $customer->name : $customer->supplier_business_name;
   $output[$key]['customer_mobile'] = $customer->mobile;

   if ($receipt_printer_type != 'printer') {
      $output[$key]['customer_info'] .= $customer->contact_address ;
       if (! empty($customer->contact_address)) {
           $output[$key]['customer_info'] .= '<br>';
       }
       
      $output[$key]['customer_info'] .= __('contact.mobile').' : '.$customer->mobile  ;
       if (! empty($customer->landline)) {
         //  $output[$key]['customer_info'] .= ', '.$customer->landline;
       }
   }

   $output[$key]['customer_tax_number'] = $customer->tax_number;
   $output[$key]['customer_tax_label'] = ! empty($il->client_tax_label) ? $il->client_tax_label : '';
   ...
   if (! empty($temp)) {
       $output[$key]['customer_custom_fields'] .= implode('<br>', $temp);
   }
   ...
   $output[$key]['customer_info_address'] = '';
   if (! empty($customer_address)) {
       $output[$key]['customer_info_address'] = implode(', ', $customer_address);
   }
}

if ($il->show_reward_point == 1) {
   $output[$key]['customer_rp_label'] = $business_details->rp_name;
   $output[$key]['customer_total_rp'] = $customer->total_rp;
}

$output[$key]['client_id'] = '';
$output[$key]['client_id_label'] = '';
if ($il->show_client_id == 1) {
   $output[$key]['client_id_label'] = ! empty($il->client_id_label) ? $il->client_id_label : '';
   $output[$key]['client_id'] = ! empty($customer->contact_id) ? $customer->contact_id : '';
}

//Sales person info
$output[$key]['sales_person'] = '';
$output[$key]['sales_person_label'] = '';
if ($il->show_sales_person == 1) {
   $output[$key]['sales_person_label'] = ! empty($il->sales_person_label) ? $il->sales_person_label : '';
   $output[$key]['sales_person'] = ! empty($transaction->sales_person->user_full_name) ? $transaction->sales_person->user_full_name : '';
}

//commission agent info
$output[$key]['commission_agent'] = '';
$output[$key]['commission_agent_label'] = '';
if ($il->show_commission_agent == 1) {
   $output[$key]['commission_agent_label'] = ! empty($il->commission_agent_label) ? $il->commission_agent_label : '';
   $output[$key]['commission_agent'] = ! empty($transaction->sale_commission_agent->user_full_name) ? $transaction->sale_commission_agent->user_full_name : '';
}

//Invoice info
$output[$key]['invoice_no'] = $transaction->invoice_no;
$output[$key]['invoice_no_prefix'] = $il->invoice_no_prefix;
// $output[$key]['shipping_address'] = ! empty($transaction->shipping_address()) ? $transaction->shipping_address() : $transaction->shipping_address;

//Heading & invoice label, when quotation use the quotation heading.
if ($transaction_type == 'sell_return') {
   $output[$key]['invoice_heading'] = $il->cn_heading;
   $output[$key]['invoice_no_prefix'] = $il->cn_no_label;

   //Parent sell details(return_parent_id)
   $output[$key]['parent_invoice_no'] = Transaction::find($transaction->return_parent_id)->invoice_no;
   $output[$key]['parent_invoice_no_prefix'] = $il->invoice_no_prefix;
} elseif ($transaction->status == 'draft' && $transaction->sub_status == 'proforma' && ! empty($il->common_settings['proforma_heading'])) {
   $output[$key]['invoice_heading'] = $il->common_settings['proforma_heading'];
} elseif ($transaction->status == 'draft' && $transaction->is_quotation == 1) {
   $output[$key]['invoice_heading'] = $il->quotation_heading;
   $output[$key]['invoice_no_prefix'] = $il->quotation_no_prefix;
} elseif ($transaction_type == 'sales_order') {
   $output[$key]['invoice_heading'] = ! empty($il->common_settings['sales_order_heading']) ? $il->common_settings['sales_order_heading'] : __('lang_v1.sales_order');
   $output[$key]['invoice_no_prefix'] = $il->quotation_no_prefix;
} else {
   $output[$key]['invoice_heading'] = $il->invoice_heading;
   if ($transaction->payment_status == 'paid' && ! empty($il->invoice_heading_paid)) {
       $output[$key]['invoice_heading'] .= ' '.$il->invoice_heading_paid;
   } elseif (in_array($transaction->payment_status, ['due', 'partial']) && ! empty($il->invoice_heading_not_paid)) {
       $output[$key]['invoice_heading'] .= ' '.$il->invoice_heading_not_paid;
   }
}

$output[$key]['date_label'] = $il->date_label;
if (blank($il->date_time_format)) {
   $output[$key]['invoice_date'] = $this->format_date($transaction->transaction_date, true, $business_details);
} else {
   $output[$key]['invoice_date'] = \Carbon::createFromFormat('Y-m-d H:i:s', $transaction->transaction_date)->format($il->date_time_format);
}

$output[$key]['transaction_date'] = $transaction->transaction_date;
$output[$key]['date_time_format'] = $business_details->date_format;
$output[$key]['currency_symbol'] = $business_details->currency_symbol;

$output[$key]['hide_price'] = ! empty($il->common_settings['hide_price']) ? true : false;

if (! empty($il->common_settings['show_due_date']) && $transaction->payment_status != 'paid') {
   $output[$key]['due_date_label'] = ! empty($il->common_settings['due_date_label']) ? $il->common_settings['due_date_label'] : '';
   $due_date = $transaction->due_date;
   if (! empty($due_date)) {
       if (blank($il->date_time_format)) {
           $output[$key]['due_date'] = $this->format_date($due_date->toDateTimeString(), true, $business_details);
       } else {
           $output[$key]['due_date'] = \Carbon::createFromFormat('Y-m-d H:i:s', $due_date->toDateTimeString())->format($il->date_time_format);
       }
   }
}
...
if (in_array($transaction_type, ['sell', 'sales_order'])) {
   ...
   $output[$key]['item_discount_label'] = $il->common_settings['item_discount_label'] ?? '';

   $output[$key]['discounted_unit_price_label'] = $il->common_settings['discounted_unit_price_label'] ?? '';

   $output[$key]['show_base_unit_details'] = ! empty($il->common_settings['show_base_unit_details']);

   $output[$key]['tax_summary_label'] = $il->common_settings['tax_summary_label'] ?? '';
  
    $details = $this->_receiptDetailsSellLines($lines, $il, $business_details);

   $output[$key]['lines'] = $details['lines'];
   $output[$key]['taxes'] = [];
   $total_quantity = 0;
   $total_line_discount = 0;
   $total_line_taxes = 0;
   ...
   foreach ($details['lines'] as $line) {
       if (! empty($line['group_tax_details'])) {
           foreach ($line['group_tax_details'] as $tax_group_detail) {
               if (! isset($output[$key]['taxes'][$tax_group_detail['name']])) {
                   $output[$key]['taxes'][$tax_group_detail['name']] = 0;
               }
               $output[$key]['taxes'][$tax_group_detail['name']] += $tax_group_detail['calculated_tax'];
           }
       } elseif (! empty($line['tax_id'])) {
           if (! isset($output[$key]['taxes'][$line['tax_name']])) {
               $output[$key]['taxes'][$line['tax_name']] = 0;
           }

           $output[$key]['taxes'][$line['tax_name']] += ($line['tax_unformatted'] * $line['quantity_uf']);
       }
   ...
   if (! empty($il->common_settings['total_quantity_label'])) {
       $output[$key]['total_quantity_label'] = $il->common_settings['total_quantity_label'];
       $output[$key]['total_quantity'] = $this->num_f($total_quantity, false, $business_details, true);
   }

   if (! empty($il->common_settings['total_items_label'])) {
       $output[$key]['total_items_label'] = $il->common_settings['total_items_label'];
       $output[$key]['total_items'] = count($unique_items);
   }

   $output[$key]['subtotal_exc_tax'] = $this->num_f($subtotal_exc_tax, true, $business_details);
   $output[$key]['total_line_discount'] = ! empty($total_line_discount) ? $this->num_f($total_line_discount, true, $business_details) : 0;
} elseif ($transaction_type == 'sell_return') {
   $parent_sell = Transaction::find($transaction->return_parent_id);
   $lines = $parent_sell->sell_lines;
...
//show cat code
$output[$key]['show_cat_code'] = $il->show_cat_code;
$output[$key]['cat_code_label'] = $il->cat_code_label;

//Subtotal
$output[$key]['subtotal_label'] = $il->sub_total_label.':';
$output[$key]['subtotal'] = ($transaction->total_before_tax != 0) ? $this->num_f($transaction->total_before_tax, $show_currency, $business_details) : 0;
$output[$key]['subtotal_unformatted'] = ($transaction->total_before_tax != 0) ? $transaction->total_before_tax : 0;

//round off
$output[$key]['round_off_label'] = ! empty($il->round_off_label) ? $il->round_off_label.':' : __('lang_v1.round_off').':';
$output[$key]['round_off'] = $this->num_f($transaction->round_off_amount, $show_currency, $business_details);
$output[$key]['round_off_amount'] = $transaction->round_off_amount;
$output[$key]['total_exempt'] = $this->num_f($total_exempt, $show_currency, $business_details);
$output[$key]['total_exempt_uf'] = $total_exempt;

$taxed_subtotal = $output[$key]['subtotal_unformatted'] - $total_exempt;
$output[$key]['taxed_subtotal'] = $this->num_f($taxed_subtotal, $show_currency, $business_details);

//Discount
$discount_amount = $this->num_f($transaction->discount_amount, $show_currency, $business_details);
$output[$key]['line_discount_label'] = $invoice_layout->discount_label;
$output[$key]['discount_label'] = $invoice_layout->discount_label;
$output[$key]['discount_label'] .= ($transaction->discount_type == 'percentage') ? ' <small>('.$this->num_f($transaction->discount_amount, false, $business_details).'%)</small> :' : '';
...
$output[$key]['discount'] = ($discount != 0) ? $this->num_f($discount, $show_currency, $business_details) : 0;

//reward points
if ($business_details->enable_rp == 1 && ! empty($transaction->rp_redeemed)) {
   $output[$key]['reward_point_label'] = $business_details->rp_name;
   $output[$key]['reward_point_amount'] = $this->num_f($transaction->rp_redeemed_amount, $show_currency, $business_details);
}

//Format tax
if (! empty($output[$key]['taxes'])) {
   $total_tax = 0;
   foreach ($output[$key]['taxes'] as $key => $value) {
       $total_tax += $value;

       $output[$key]['taxes'][$key] = $this->num_f($value, $show_currency, $business_details);
   }

   $output[$key]['taxes'][trans('lang_v1.total_tax')] = $this->num_f($total_tax, $show_currency, $business_details);
}

//Order Tax
$tax = $transaction->tax;
$output[$key]['tax_label'] = $invoice_layout->tax_label;
$output[$key]['line_tax_label'] = $invoice_layout->tax_label;
if (! empty($tax) && ! empty($tax->name)) {
   $output[$key]['tax_label'] .= ' ('.$tax->name.')';
}
$output[$key]['tax_label'] .= ':';
$output[$key]['tax'] = ($transaction->tax_amount != 0) ? $this->num_f($transaction->tax_amount, $show_currency, $business_details) : 0;

if ($transaction->tax_amount != 0 && $tax->is_tax_group) {
   $transaction_group_tax_details = $this->groupTaxDetails($tax, $transaction->tax_amount);

   $output[$key]['group_tax_details'] = [];
   foreach ($transaction_group_tax_details as $value) {
       $output[$key]['group_tax_details'][$value['name']] = $this->num_f($value['calculated_tax'], $show_currency, $business_details);
   }
}

//Shipping charges
$output[$key]['shipping_charges'] = ($transaction->shipping_charges != 0) ? $this->num_f($transaction->shipping_charges, $show_currency, $business_details) : 0;
$output[$key]['shipping_charges_label'] = trans('sale.shipping_charges');
//Shipping details
$output[$key]['shipping_details'] = $transaction->shipping_details;
$output[$key]['delivered_to'] = $transaction->delivered_to;
$output[$key]['shipping_details_label'] = trans('sale.shipping_details');
$output[$key]['packing_charge_label'] = trans('lang_v1.packing_charge');
$output[$key]['packing_charge'] = ($transaction->packing_charge != 0) ? $this->num_f($transaction->packing_charge, $show_currency, $business_details) : 0;

//Total
if ($transaction_type == 'sell_return') {
   $output[$key]['total_label'] = $invoice_layout->cn_amount_label.':';
   $output[$key]['total'] = $this->num_f($transaction->final_total, $show_currency, $business_details);
} else {
   $output[$key]['total_label'] = $invoice_layout->total_label.':';
   $output[$key]['total'] = $this->num_f($transaction->final_total, $show_currency, $business_details);
}
if (! empty($il->common_settings['show_total_in_words'])) {
   $word_format = isset($il->common_settings['num_to_word_format']) ? $il->common_settings['num_to_word_format'] : 'international';
   $output[$key]['total_in_words'] = $this->numToWord($transaction->final_total, null, $word_format);
}

$output[$key]['total_unformatted'] = $transaction->final_total;
...
if ($transaction_type == 'sell' && $transaction->status == 'final') {
   ...
   $output[$key]['total_paid'] = ($paid_amount == 0) ? 0 : $this->num_f($paid_amount, $show_currency, $business_details);
   $output[$key]['total_paid_label'] = $il->paid_label;
   $output[$key]['total_due'] = ($due == 0) ? 0 : $this->num_f($due, $show_currency, $business_details);
   $output[$key]['total_due_label'] = $il->total_due_label;

   if ($il->show_previous_bal == 1) {
       $all_due = $this->getContactDue($transaction->contact_id);
       if (! empty($all_due)) {
           $output[$key]['all_bal_label'] = $il->prev_bal_label;
           $output[$key]['all_due'] = $this->num_f($all_due, $show_currency, $business_details);
       }
   }
...
$output[$key]['additional_expenses'] = [];

if (! empty($transaction->additional_expense_value_1) && ! empty($transaction->additional_expense_key_1)) {
   $output[$key]['additional_expenses'][$transaction->additional_expense_key_1] = $this->num_f($transaction->additional_expense_value_1, $show_currency, $business_details);
}
if (! empty($transaction->additional_expense_value_2) && ! empty($transaction->additional_expense_key_2)) {
   $output[$key]['additional_expenses'][$transaction->additional_expense_key_2] = $this->num_f($transaction->additional_expense_value_2, $show_currency, $business_details);
}
if (! empty($transaction->additional_expense_value_3) && ! empty($transaction->additional_expense_key_3)) {
   $output[$key]['additional_expenses'][$transaction->additional_expense_key_3] = $this->num_f($transaction->additional_expense_value_3, $show_currency, $business_details);
}
if (! empty($transaction->additional_expense_value_4) && ! empty($transaction->additional_expense_key_4)) {
   $output[$key]['additional_expenses'][$transaction->additional_expense_key_4] = $this->num_f($transaction->additional_expense_value_4, $show_currency, $business_details);
}

//Check for barcode
$output[$key]['barcode'] = ($il->show_barcode == 1) ? $transaction->invoice_no : false;

//Additional notes
$output[$key]['additional_notes'] = $transaction->additional_notes;
$output[$key]['footer_text'] = $invoice_layout->footer_text;

//Barcode related information.
$output[$key]['show_barcode'] = ! empty($il->show_barcode) ? true : false;

if (in_array($transaction_type, ['sell', 'sales_order'])) {
   //Qr code related information.
   $output[$key]['show_qr_code'] = ! empty($il->show_qr_code) ? true : false;
   ...
   if ($zatca_qr) {
       ...
       if (in_array('address', $qr_code_fields)) {
           $qr_code_details[] = $is_label_enabled ? __('business.address').': '.$location_details->name.', '.$output[$key]['address'] : $location_details->name.' '.str_replace(',', '', $output[$key]['address']);
       }
       if (in_array('tax_1', $qr_code_fields)) {
           $qr_code_details[] = $is_label_enabled ? $business_details->tax_label_1.': '.$business_details->tax_number_1 : $business_details->tax_number_1;
       }
       ...
       if (in_array('invoice_datetime', $qr_code_fields)) {
           $qr_code_details[] = $is_label_enabled ? $output[$key]['date_label'].': '.$output[$key]['invoice_date'] : $output[$key]['invoice_date'];
       }
       if (in_array('subtotal', $qr_code_fields)) {
           $qr_code_details[] = $is_label_enabled ? $output[$key]['subtotal_label'].' '.$output[$key]['subtotal'] : $output[$key]['subtotal'];
       }
       if (in_array('total_amount', $qr_code_fields)) {
           $qr_code_details[] = $is_label_enabled ? $output[$key]['total_label'].' '.$output[$key]['total'] : $output[$key]['total'];
       }
       if (in_array('total_tax', $qr_code_fields)) {
           $total_order_tax = $transaction->tax_amount + $total_line_taxes;
           $total_order_tax_formatted = $this->num_f($total_order_tax, $show_currency, $business_details);
       ...
       $output[$key]['qr_code_details'] = $qr_code_details;

       $qr_code_text = $is_label_enabled ? implode(', ', $qr_code_details) : implode(' ', $qr_code_details);
   }

   if ($transaction->status == 'final') {
       $output[$key]['qr_code_text'] = $qr_code_text;
   }
}
//Module related information.
$il->module_info = ! empty($il->module_info) ? json_decode($il->module_info, true) : [];
if (! empty($il->module_info['tables']) && $this->isModuleEnabled('tables')) {
   //Table label & info
   $output[$key]['table_label'] = null;
   $output[$key]['table'] = null;
   if (isset($il->module_info['tables']['show_table'])) {
       $output[$key]['table_label'] = ! empty($il->module_info['tables']['table_label']) ? $il->module_info['tables']['table_label'] : '';
       if (! empty($transaction->res_table_id)) {
           $table = ResTable::find($transaction->res_table_id);
       }

       //res_table_id
       $output[$key]['table'] = ! empty($table->name) ? $table->name : '';
   }
}

if (! empty($il->module_info['types_of_service']) && $this->isModuleEnabled('types_of_service') && ! empty($transaction->types_of_service_id)) {
   //Table label & info
   $output[$key]['types_of_service_label'] = null;
   $output[$key]['types_of_service'] = null;
   if (isset($il->module_info['types_of_service']['show_types_of_service'])) {
       $output[$key]['types_of_service_label'] = ! empty($il->module_info['types_of_service']['types_of_service_label']) ? $il->module_info['types_of_service']['types_of_service_label'] : '';
       $output[$key]['types_of_service'] = $transaction->types_of_service->name;
   }

   if (isset($il->module_info['types_of_service']['show_tos_custom_fields'])) {
       $types_of_service_custom_labels = $this->getCustomLabels($business_details, 'types_of_service');
       $output[$key]['types_of_service_custom_fields'] = [];
       if (! empty($transaction->service_custom_field_1)) {
           $tos_custom_label_1 = $types_of_service_custom_labels['custom_field_1'] ?? __('lang_v1.service_custom_field_1');
           $output[$key]['types_of_service_custom_fields'][$tos_custom_label_1] = $transaction->service_custom_field_1;
       }
       if (! empty($transaction->service_custom_field_2)) {
           $tos_custom_label_2 = $types_of_service_custom_labels['custom_field_2'] ?? __('lang_v1.service_custom_field_2');
           $output[$key]['types_of_service_custom_fields'][$tos_custom_label_2] = $transaction->service_custom_field_2;
       }
       if (! empty($transaction->service_custom_field_3)) {
           $tos_custom_label_3 = $types_of_service_custom_labels['custom_field_3'] ?? __('lang_v1.service_custom_field_3');
           $output[$key]['types_of_service_custom_fields'][$tos_custom_label_3] = $transaction->service_custom_field_3;
       }
       if (! empty($transaction->service_custom_field_4)) {
           $tos_custom_label_4 = $types_of_service_custom_labels['custom_field_4'] ?? __('lang_v1.service_custom_field_4');
           $output[$key]['types_of_service_custom_fields'][$tos_custom_label_4] = $transaction->service_custom_field_4;
       }

       if (! empty($transaction->service_custom_field_5)) {
           $tos_custom_label_5 = $types_of_service_custom_labels['custom_field_5'] ?? __('lang_v1.service_custom_field_5');
           $output[$key]['types_of_service_custom_fields'][$tos_custom_label_5] = $transaction->service_custom_field_5;
       }

       if (! empty($transaction->service_custom_field_6)) {
           $tos_custom_label_6 = $types_of_service_custom_labels['custom_field_6'] ?? __('lang_v1.service_custom_field_6');
           $output[$key]['types_of_service_custom_fields'][$tos_custom_label_6] = $transaction->service_custom_field_6;
       }
   }
}
...
if (! empty($il->module_info['service_staff']) && $this->isModuleEnabled('service_staff')) {
   //Waiter label & info
   $output[$key]['service_staff_label'] = null;
   $output[$key]['service_staff'] = null;
   if (isset($il->module_info['service_staff']['show_service_staff'])) {
       $output[$key]['service_staff_label'] = ! empty($il->module_info['service_staff']['service_staff_label']) ? $il->module_info['service_staff']['service_staff_label'] : '';
       if (! empty($transaction->res_waiter_id)) {
           $waiter = \App\User::find($transaction->res_waiter_id);
       }

       //res_table_id
       $output[$key]['service_staff'] = ! empty($waiter->id) ? implode(' ', [$waiter->first_name, $waiter->last_name]) : '';
   }
}
...
if (! empty($il->module_info['repair']) && $transaction->sub_type == 'repair') {
   if (! empty($il->module_info['repair']['show_repair_status'])) {
       $output[$key]['repair_status_label'] = $il->module_info['repair']['repair_status_label'];
       $output[$key]['repair_status'] = '';
       if (! empty($transaction->repair_status_id)) {
           $repair_status = \Modules\Repair\Entities\RepairStatus::find($transaction->repair_status_id);
           $output[$key]['repair_status'] = $repair_status->name;
       }
   }

   if (! empty($il->module_info['repair']['show_repair_warranty'])) {
       $output[$key]['repair_warranty_label'] = $il->module_info['repair']['repair_warranty_label'];
       $output[$key]['repair_warranty'] = '';
       if (! empty($transaction->repair_warranty_id)) {
           $repair_warranty = \App\Warranty::find($transaction->repair_warranty_id);
           $output[$key]['repair_warranty'] = $repair_warranty->name;
       }
   }

   if (! empty($il->module_info['repair']['show_serial_no'])) {
       $output[$key]['serial_no_label'] = $il->module_info['repair']['serial_no_label'];
       $output[$key]['repair_serial_no'] = $transaction->repair_serial_no;
   }

   if (! empty($il->module_info['repair']['show_defects'])) {
       $output[$key]['defects_label'] = $il->module_info['repair']['defects_label'];
       $output[$key]['repair_defects'] = $transaction->repair_defects;
   }

   if (! empty($il->module_info['repair']['show_model'])) {
       $output[$key]['model_no_label'] = $il->module_info['repair']['model_no_label'];

       $output[$key]['repair_model_no'] = '';

       if (! empty($transaction->repair_model_id)) {
           $device_model = \Modules\Repair\Entities\DeviceModel::find($transaction->repair_model_id);

           if (! empty($device_model)) {
               $output[$key]['repair_model_no'] = $device_model->name;
           }
       }
   }

   if (! empty($il->module_info['repair']['show_repair_checklist'])) {
       $output[$key]['repair_checklist_label'] = $il->module_info['repair']['repair_checklist_label'];
       $output[$key]['checked_repair_checklist'] = $transaction->repair_checklist;
       ...
       $output[$key]['repair_checklist'] = $checklists;
   }

   if (! empty($il->module_info['repair']['show_device'])) {
       $output[$key]['device_label'] = $il->module_info['repair']['device_label'];
       $device = \App\Category::find($transaction->repair_device_id);

       $output[$key]['repair_device'] = '';
       if (! empty($device)) {
           $output[$key]['repair_device'] = $device->name;
       }
   }

   if (! empty($il->module_info['repair']['show_brand'])) {
       $output[$key]['brand_label'] = $il->module_info['repair']['brand_label'];
       $brand = \App\Brands::find($transaction->repair_brand_id);
       $output[$key]['repair_brand'] = '';
       if (! empty($brand)) {
           $output[$key]['repair_brand'] = $brand->name;
       }
   }
}
...
//shipping custom fields
if (! empty($custom_labels->shipping->custom_field_1)) {
   $output[$key]['shipping_custom_field_1_label'] = $custom_labels->shipping->custom_field_1;
   $output[$key]['shipping_custom_field_1_value'] = $transaction['shipping_custom_field_1'];
}

if (! empty($custom_labels->shipping->custom_field_2)) {
   $output[$key]['shipping_custom_field_2_label'] = $custom_labels->shipping->custom_field_2;
   $output[$key]['shipping_custom_field_2_value'] = $transaction['shipping_custom_field_2'];
}

if (! empty($custom_labels->shipping->custom_field_3)) {
   $output[$key]['shipping_custom_field_3_label'] = $custom_labels->shipping->custom_field_3;
   $output[$key]['shipping_custom_field_3_value'] = $transaction['shipping_custom_field_3'];
}

if (! empty($custom_labels->shipping->custom_field_4)) {
   $output[$key]['shipping_custom_field_4_label'] = $custom_labels->shipping->custom_field_4;
   $output[$key]['shipping_custom_field_4_value'] = $transaction['shipping_custom_field_4'];
}

//Sell custom fields
if (! empty($custom_labels->shipping->custom_field_5)) {
   $output[$key]['shipping_custom_field_5_label'] = $custom_labels->shipping->custom_field_5;
   $output[$key]['shipping_custom_field_5_value'] = $transaction['shipping_custom_field_5'];
}


$is_show_sell_custom_fields1 = ! empty($il->common_settings['sell_custom_fields1']) ? true : false;
if (! empty($custom_labels->sell->custom_field_1) && $is_show_sell_custom_fields1) {
   $output[$key]['sell_custom_field_1_label'] = $custom_labels->sell->custom_field_1;
   $output[$key]['sell_custom_field_1_value'] = $transaction['custom_field_1'];
}

$is_show_sell_custom_fields2 = ! empty($il->common_settings['sell_custom_fields2']) ? true : false;
if (! empty($custom_labels->sell->custom_field_2) && $is_show_sell_custom_fields2) {
   $output[$key]['sell_custom_field_2_label'] = $custom_labels->sell->custom_field_2;
   $output[$key]['sell_custom_field_2_value'] = $transaction['custom_field_2'];
}

$is_show_sell_custom_fields3 = ! empty($il->common_settings['sell_custom_fields3']) ? true : false;
if (! empty($custom_labels->sell->custom_field_3) && $is_show_sell_custom_fields3) {
   $output[$key]['sell_custom_field_3_label'] = $custom_labels->sell->custom_field_3;
   $output[$key]['sell_custom_field_3_value'] = $transaction['custom_field_3'];
}

$is_show_sell_custom_fields4 = ! empty($il->common_settings['sell_custom_fields4']) ? true : false;
if (! empty($custom_labels->sell->custom_field_4) && $is_show_sell_custom_fields4) {
   $output[$key]['sell_custom_field_4_label'] = $custom_labels->sell->custom_field_4;
   $output[$key]['sell_custom_field_4_value'] = $transaction['custom_field_4'];
}
...
// location custom fields
if (in_array('custom_field1', $location_custom_field_settings) && ! empty($location_details->custom_field1) && ! empty($custom_labels->location->custom_field_1)) {
   $output[$key]['location_custom_field_1_label'] = $custom_labels->location->custom_field_1;
   $output[$key]['location_custom_field_1_value'] = $location_details->custom_field1;
}

if (in_array('custom_field2', $location_custom_field_settings) && ! empty($location_details->custom_field2) && ! empty($custom_labels->location->custom_field_2)) {
   $output[$key]['location_custom_field_2_label'] = $custom_labels->location->custom_field_2;
   $output[$key]['location_custom_field_2_value'] = $location_details->custom_field2;
}

if (in_array('custom_field3', $location_custom_field_settings) && ! empty($location_details->custom_field3) && ! empty($custom_labels->location->custom_field_3)) {
   $output[$key]['location_custom_field_3_label'] = $custom_labels->location->custom_field_3;
   $output[$key]['location_custom_field_3_value'] = $location_details->custom_field3;
}

if (in_array('custom_field4', $location_custom_field_settings) && ! empty($location_details->custom_field4) && ! empty($custom_labels->location->custom_field_4)) {
   $output[$key]['location_custom_field_4_label'] = $custom_labels->location->custom_field_4;
   $output[$key]['location_custom_field_4_value'] = $location_details->custom_field4;
}
...
if (! empty($transaction->sales_order_ids)) {
   ...
   $output[$key]['sale_orders_invoice_no'] = implode(', ', $sale_orders->pluck('invoice_no')->toArray());
   $sale_orders_invoice_date = [];
   foreach ($sale_orders as $sale_order) {
       if (blank($il->date_time_format)) {
           $sale_orders_invoice_date[] = $this->format_date($sale_order->transaction_date, true);
   ...
   $output[$key]['sale_orders_invoice_date'] = implode(', ', $sale_orders_invoice_date);
}

if (! empty($transaction->prefer_payment_method)) {
   $payment_types = $this->payment_types(null, true, $transaction->business_id);
   $output[$key]['preferred_payment_method'] = $payment_types[$transaction->prefer_payment_method];
}

if (! empty($transaction->prefer_payment_account)) {
   $output[$key]['preferred_account_details'] = $transaction->preferredAccount->account_details;
}

//export custom fields
$output[$key]['is_export'] = $transaction->is_export;
$export_custom_fields_info = $transaction->export_custom_fields_info;
if (! empty($transaction->is_export)) {
   $output[$key]['export_custom_fields_info']['export_custom_field_1'] = $export_custom_fields_info['export_custom_field_1'] ?? '';
   $output[$key]['export_custom_fields_info']['export_custom_field_2'] = $export_custom_fields_info['export_custom_field_2'] ?? '';
   $output[$key]['export_custom_fields_info']['export_custom_field_3'] = $export_custom_fields_info['export_custom_field_3'] ?? '';
   $output[$key]['export_custom_fields_info']['export_custom_field_4'] = $export_custom_fields_info['export_custom_field_4'] ?? '';
   $output[$key]['export_custom_fields_info']['export_custom_field_5'] = $export_custom_fields_info['export_custom_field_5'] ?? '';
   $output[$key]['export_custom_fields_info']['export_custom_field_6'] = $export_custom_fields_info['export_custom_field_6'] ?? '';
}

$output[$key]['design'] = $il->design;
$output[$key]['table_tax_headings'] = ! empty($il->table_tax_headings) ? array_filter(json_decode($il->table_tax_headings), 'strlen') : null;
...
return  $output;
         ...
         protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
     ...
     public function _receiptDetailsSellLinescopy($lines, $il, $business_details)
    {
        $is_lot_number_enabled = $business_details->enable_lot_number;
        $is_product_expiry_enabled = $business_details->enable_product_expiry;
    ...
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
                    ...
                    ->select(
                        ...
                        DB::raw('SUM(sale_commision) as sale_commision'),
                        DB::raw('SUM(final_total - tax_amount) as total_exc_tax'),
       ...
       // }
        ...
        $output['sale_commision'] = $sell_details->sale_commision;
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [], $permitted_locations = null)
    {
        ...
        public function getOpeningClosingStockhome($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [], $permitted_locations = null)
    {
        $query = PurchaseLine::join(
            'transactions as purchase',
            'purchase_lines.transaction_id',
    ...
    public function getTransactionTotals(
        ...
        if (in_array('expense', $transaction_types)) {
           ...
           //     DB::raw("SUM(IF(transactions.type='expense', final_total, 0)) as total_expense")  
            DB::raw("SUM(IF(transactions.type='expense' AND transactions.expense_category_id = 25, final_total, 0)) as total_expense")
  ...
  $query2 = Transaction::leftJoin('expense_categories AS ec', 'transactions.expense_category_id', '=', 'ec.id')
                        ...
                        ->where('transactions.expense_category_id', 25)
                        ->where('transactions.business_id', $business_id);
    if (!empty($start_date) && !empty($end_date)) {
        $query2->whereDate('transactions.transaction_date', '>=', $start_date)
            ->whereDate('transactions.transaction_date', '<=', $end_date);
    }
    if (empty($start_date) && !empty($end_date)) {
    ...
    $transaction_totals1 = $query2->first();
   
    $total_expense1 = !empty($transaction_totals1->total_expense) ? $transaction_totals1->total_expense : 0;
    $total_expense_refund1 = !empty($transaction_totals1->total_expense_refund) ? $transaction_totals1->total_expense_refund : 0;
   
     $output['total_expense_1'] =  $total_expense1 - $total_expense_refund1; ;
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->select(
                    'transactions.id',
                    'transactions.sale_commision',
                    'transactions.commision_status',
                    'transactions.transaction_date',
                    'transactions.type',
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null, $permitted_locations = null)
    {
        ...
        $data['sale_commision'] = ! empty($sell_details['sale_commision']) ? $sell_details['sale_commision'] : 0;
        ...
        $data['total_expense_1'] = $transaction_totals['total_expense_1'];
       ...
       $data['$module_total']  = $module_total;
       $data['$gross_profit']  = $gross_profit;
  
        $data['net_profit'] = $module_total + $gross_profit - $data['sale_commision'] 
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        if ($due_payment_type == 'sell' && $is_reverse) {
            $payment_type = 'debit'; //customer 
        }
        if ($due_payment_type == 'purchase' && $is_reverse) {
            $payment_type = 'credit'; /// supplier 
        }

         if ($due_payment_type == 'sell' && $request->paytype == 'payreturn') {
            $payment_type = 'debit';
            $inputs['is_advance'] = 2;

        }
        
         if ($due_payment_type == 'purchase' && $request->paytype == 'payreturn') {
            $payment_type = 'credit';
            $inputs['is_advance'] = 2;
    ...
    public function registerReport($business_id, $permitted_locations, $start_date = null, $end_date = null)
    {
        $registers = CashRegister::leftjoin(
            ...
            ->select(
                ...
                DB::raw("SUM(IF(pay_method='custom_pay_8', IF(transaction_type='sell', amount, 0), 0)) as total_custom_pay_8"),
                DB::raw("SUM(IF(pay_method='custom_pay_9', IF(transaction_type='sell', amount, 0), 0)) as total_custom_pay_9"),
                DB::raw("SUM(IF(pay_method='custom_pay_10', IF(transaction_type='sell', amount, 0), 0)) as total_custom_pay_10")
