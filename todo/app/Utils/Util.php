...
class Util
{
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        $payment_types['custom_pay_8'] = ! empty($custom_labels['payments']['custom_pay_8']) ? $custom_labels['payments']['custom_pay_8'] : __('lang_v1.custom_payment', ['number' => 8]);
        $payment_types['custom_pay_9'] = ! empty($custom_labels['payments']['custom_pay_9']) ? $custom_labels['payments']['custom_pay_9'] : __('lang_v1.custom_payment', ['number' => 9]);
        $payment_types['custom_pay_10'] = ! empty($custom_labels['payments']['custom_pay_10']) ? $custom_labels['payments']['custom_pay_10'] : __('lang_v1.custom_payment', ['number' => 10]);
    ...
    /**
     * Get all admin user IDs for a business
     *
     * @param  int  $business_id
     * @return array
     */
    public function getAdminUserIds($business_id)
    {
        $admin_role = 'Admin#'.$business_id;
        $admin_users = \App\User::where('business_id', $business_id)
            ->whereHas('roles', function ($query) use ($admin_role) {
