...
<section class="content-header">
    <h1>@lang('Project Checklist & Tasks')
...
<div class="modal fade" id="editTaskModal" tabindex="-1" role="dialog" aria-labelledby="editTaskModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="edit_task_form" onsubmit="return handleEditTaskSubmit(event);">
                <div class="modal-body">
                    <input type="hidden" name="task_id" id="edit_task_id">

                    <div class="form-group">
                        <label for="edit_task_name">@lang('Task Name')</label>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="edit_start_date">@lang('Start Date')</label>
                                <input type="text" class="form-control date-picker" name="start_date" id="edit_start_date" placeholder="@lang('Select start date')" readonly>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="edit_end_date">@lang('End Date')</label>
                                <input type="text" class="form-control date-picker" name="end_date" id="edit_end_date" placeholder="@lang('Select end date')" readonly>
                            </div>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="edit_user_id">@lang('Assigned To')</label>
                        <select class="form-control select2" name="user_id" id="edit_user_id">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="remark" id="edit_task_remark" rows="3"></textarea>
                    </div>
...
<script type="text/javascript">
    // Define global function for edit modal - MUST be before $(document).ready
    function editTaskNow(taskData) {
        console.log('Edit task:', taskData);
        
        // Set all form values
        $('#edit_task_id').val(taskData.id);
        $('#edit_task_name').val(taskData.name);
        $('#edit_task_remark').val(taskData.remark);
        $('#edit_task_status').prop('checked', taskData.status == 1);
        $('#edit_start_date').val(taskData.start_date);
        $('#edit_end_date').val(taskData.end_date);
        $('#edit_user_id').val(taskData.user_id);
    ...
    $(document).ready(function() {
        ...
        var project_tasks_table = $('#project_tasks_table').DataTable({
            ...
            drawCallback: function(settings) {
                // Store original state of all checkboxes after table draw
            ...
            buttons: [
                {
                    ...
                    exportOptions: {
                        // Export Sr No, Task Name, Status, Start Date, End Date, User Name, Department, Task-Update, Remark and Timeline (exclude Action column)
        ...
        $(document).on('change', '.task-status', function() {
            ...
            // Prevent any changes if checkbox is disabled (non-admin/non-project-lead users)
            if (checkbox.is(':disabled')) {
                var originalState = checkbox.data('original-state');
                checkbox.prop('checked', originalState === true);
                toastr.error('@lang("Only admin and project lead can change task status.")');
                return;
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        var taskUpdateCell = row.find('td').eq(7); // Task-Update is 8th column (index 7)
                        ...
                        // Store the new state
                        checkbox.data('original-state', status == 1);
        ...
        // Auto-save remark when edited directly in table
        ...
        // Initialize Select2 and datepickers when modal is shown
        $('#editTaskModal').on('shown.bs.modal', function() {
            // Initialize or refresh Select2 for the edit modal
            if ($('#edit_user_id').hasClass('select2-hidden-accessible')) {
            ...
            // Initialize date pickers in edit modal if not already initialized
            if (!$('#edit_start_date').data('datepicker')) {
        ...
        // Edit form submission is handled by inline handleEditTaskSubmit function
