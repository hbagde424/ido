  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
            ...
            <div class="form-group">
                ...
                <div class="form-group"> 
               {!! Form::label('payment_method', __( 'account.payment_method' ) .":*") !!}
                    <select name="payment_method" class="form-control select2">\
                        <option>@lang('messages.please_select')</option>
                        @foreach($payment_types as $account_type) 
                        <option <?php if($account_transaction->payment_method == $account_type ){ echo "selected";} ?>  value="{{$account_type}}">{{$account_type}}  </option> 
                        @endforeach
                    </select>
                </div>
