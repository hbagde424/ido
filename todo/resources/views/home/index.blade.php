...
<section class="content content-custom no-print">
    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">@lang('lang_v1.calendar')</h3>
                </div>
                <div class="box-body">
        ...
        <div class="col-md-4">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">@lang('essentials::lang.attendance')</h3>
                </div>
                <div class="box-body text-center">
                    @if(!empty($attendance))
                        <h4 class="text-success"><i class="fas fa-check-circle"></i> @lang('essentials::lang.clock_in'): {{ @format_datetime($attendance->clock_in_time) }}</h4>
                        @if(!empty($attendance->clock_out_time))
                            <h4 class="text-danger"><i class="fas fa-sign-out-alt"></i> @lang('essentials::lang.clock_out'): {{ @format_datetime($attendance->clock_out_time) }}</h4>
                        @else
                            <h4 class="text-warning"><i class="fas fa-clock"></i> @lang('essentials::lang.clock_out'): --</h4>
                        @endif
                    @else
                        <h4 class="text-muted"><i class="fas fa-exclamation-circle"></i> @lang('essentials::lang.not_clocked_in')</h4>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="col-md-4">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title"><i class="fas fa-birthday-cake text-danger"></i> @lang('lang_v1.birthdays')</h3>
                </div>
                <div class="box-body">
                    @if($birthdays->count() > 0)
                        <ul class="list-group">
                            @foreach($birthdays as $birthday_user)
                                <li class="list-group-item">
                                    <strong>{{ $birthday_user->user_full_name }}</strong>
                                    <span class="pull-right">
                                        <a href="mailto:{{ $birthday_user->email }}" class="btn btn-xs btn-info" title="Send Email"><i class="fas fa-envelope"></i></a>
                                    </span>
                                </li>
                            @endforeach
                        </ul>
                    @else
                        <p class="text-center text-muted">@lang('lang_v1.no_birthdays_today')</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">@lang('lang_v1.notifications')</h3>
                </div>
                <div class="box-body">
                    <ul class="list-group" id="dashboard_notifications_list" style="max-height: 300px; overflow-y: auto;">
                        <!-- Notifications will be loaded here -->
                        <li class="text-center"><i class="fas fa-sync fa-spin"></i> Loading...</li>
                    </ul>
                </div>
            </div>
        </div>
...
<!--    </script>-->
<style>
    #dashboard_notifications_list .notification-li {
        list-style: none;
        border-bottom: 1px solid #f4f4f4;
        padding: 10px 0;
    }
    #dashboard_notifications_list .notification-li:last-child {
        border-bottom: none;
    }
    #dashboard_notifications_list .notification-li a {
        display: flex;
        align-items: flex-start;
        text-decoration: none;
        color: #333;
    }
    #dashboard_notifications_list .notif-icon {
        margin-right: 15px;
        font-size: 24px;
        width: 40px;
        height: 40px;
    ...
    /* Default colors for icons if not set by class */
    #dashboard_notifications_list .notif-icon.bg-aqua { background-color: #00c0ef !important; }
    #dashboard_notifications_list .notif-icon.bg-green { background-color: #00a65a !important; }
    #dashboard_notifications_list .notif-icon.bg-yellow { background-color: #f39c12 !important; }
    #dashboard_notifications_list .notif-icon.bg-red { background-color: #dd4b39 !important; }

    #dashboard_notifications_list .notif-info {
        flex: 1;
        font-size: 14px;
        line-height: 1.4;
    }
    #dashboard_notifications_list .time {
        font-size: 12px;
        color: #999;
        margin-top: 5px;
        display: block;
    }
    #dashboard_notifications_list .unread {
...
<script>
    $(document).ready(function() {
        console.log("Dashboard notification script loaded (inline)");
        ...
        $.get('/get-total-unread', function(data) {
            console.log("Initial unread count data:", data);
            if(data && data.total_unread !== undefined) {
                currentUnreadCount = parseInt(data.total_unread);
            } else {
                currentUnreadCount = parseInt(data); // Fallback if it returns just a number
            }
        });
        ...
        function loadDashboardNotifications() {
            ...
            $.ajax({
                url: '/load-more-notifications?page=1',
                dataType: 'html',
                success: function(result) {
                    console.log("Notifications loaded successfully");
                    $('#dashboard_notifications_list').html(result);
                },
                error: function(xhr, status, error) {
                    console.error("Error loading notifications:", error);
                    $('#dashboard_notifications_list').html('<li class="text-center text-danger">Error loading notifications. Please try refreshing.</li>');
                }
            });
        }
        ...
        function checkNewNotifications() {
            $.get('/get-total-unread', function(data) {
                var newCount = 0;
                if(data && data.total_unread !== undefined) {
                    newCount = parseInt(data.total_unread);
                } else {
                    newCount = parseInt(data);
                }
                
                if (newCount > currentUnreadCount) {
                    console.log("New notification detected! Old:", currentUnreadCount, "New:", newCount);
                    // Play audio
                    var audio = new Audio('{{ asset("audio/success.ogg") }}');
                    ...
                    // Just update count if it decreased (read elsewhere)
                    currentUnreadCount = newCount;
                    loadDashboardNotifications();
                }
            });
