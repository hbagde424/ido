...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		@php
			...
			if(!empty($so_line) && $action !== 'edit') {
				...
				$unit_commision_inc_tax = $product->pcommission;
				$unit_total_commision_inc_tax = $product->commission_subtotal;
	...
	<td>
		...
		@foreach($sub_units as $key => $value)
        	...
        	@endif
        @endforeach
		<div class="input-group input-number" style="width:125px;">
	...
	<td>
        <input type="text" name="products[{{$row_count}}][unit_commision_inc_tax]" class="form-control pos_unit_commision_inc_tax input_number" value="{{ $unit_commision_inc_tax}}"  >
	</td>
	
	<td>
        <input type="text" name="products[{{$row_count}}][unit_total_commision_inc_tax]" class="form-control pos_unit_total_commision_inc_tax input_number" value="{{ $unit_total_commision_inc_tax}}" >
	</td>
