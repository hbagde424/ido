...
@php
    ...
    $user_media = (isset($user) && $user && method_exists($user, 'allMedia')) ? $user->allMedia : collect([]);
@endphp
@foreach($document_types as $doc_key => $doc_label)
<div class="form-group col-md-6">
    {!! Form::label($doc_key, $doc_label . ':') !!}
    {!! Form::file($doc_key, ['class' => 'form-control document-upload', 'data-doc-type' => $doc_key, 'accept' => 'image/*,.pdf,.doc,.docx']) !!}
    <div class="document-preview mt-2" id="preview_{{ $doc_key }}">
        @if(isset($user) && $user)
            @php
                $existing_doc = $user_media->where('model_media_type', $doc_key)->first();
            @endphp
            @if($existing_doc)
                <div class="existing-document">
                    <a href="{{ $existing_doc->display_url }}" target="_blank" class="btn btn-sm btn-info">
                        <i class="fa fa-eye"></i> @lang('lang_v1.view')
                    </a>
                    <span class="document-name">{{ $existing_doc->display_name }}</span>
                </div>
            @endif
        @endif
    </div>
