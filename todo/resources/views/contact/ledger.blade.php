...
<div class="col-md-12 col-sm-12 @if(!empty($for_pdf)) width-100 align-right @endif">
        <p class="text-right align-right"><strong>{{$location->name}}</strong>
        	<br>
        	@if(!empty($location))
...
<div class="col-md-6 col-sm-6 col-xs-6 text-right align-right @if(!empty($for_pdf)) width-50 f-left @endif">
	...
	@endphp
			...
			<?php $rrbalance=$data['balance']; ?>
			@endforeach
		<td><strong>@lang('lang_v1.balance_due')</strong></td>
		<!--<td class="align-right">@format_currency($ledger_details['total_invoice']- $ledger_details['total_paid'])</td>-->
	...
	<table class="table table-striped @if(!empty($for_pdf)) table-pdf td-border @endif" id="ledger_table">
		<thead>
			<tr class="row-border blue-heading">
				<th width="15%" class="text-left">@lang('lang_v1.date')</th>
				<!--<th width="9%" class="text-center">@lang('purchase.ref_no')</th>-->
				<!--<th width="22%" class="text-center">@lang('lang_v1.type')</th>-->
				<th width="25%" class="text-left">Description</th>
					<th width="10%" class="text-left">Due Days</th>
				<!--<th width="10%" class="text-center">@lang('sale.location')</th>-->
				<!--<th width="5%" class="text-left">@lang('sale.payment_status')</th>-->
				{{--<th width="15%" class="text-left">@lang('sale.total')</th>--}}
				<th width="15%" class="text-left">@lang('account.debit')</th>
				<th width="15%" class="text-left">@lang('account.credit')</th>
				<th width="20%" class="text-left">@lang('lang_v1.balance')</th>
				<!--<th width="5%" class="text-center">@lang('lang_v1.payment_method')</th>-->
			
				<!--<th width="25%" class="text-left">@lang('report.others')</th>-->
			</tr>
	...
	@endphp 
			...
			@foreach($ledger_details['ledger'] as $data)
				<tr @if(!empty($for_pdf) && $loop->iteration % 2 == 0) class="odd" @endif>
					...
					<!--<td></td>-->
					<td>{{$data['type']}} No. {{$data['ref_no']}}   {{$data['others']}}  </td>
						<td class="text-left"> 
					    <?php
					    
					    if($data['type']!='Payment'){
					    if($data['payment_status'] == 'Due'  || $data['payment_status'] == 'Partial'  ){
  $date1 = Carbon::parse(($data['date']));
   $date2 = Carbon::now();
...
echo  $diffInDays;
					...
					{{--<td class="ws-nowrap align-center">@if($data['total'] !== '') @format_currency($data['total']) @endif</td>--}}
					<td class="ws-nowrap align-center">@if($data['debit'] != '') @format_currency($data['debit']) @endif</td>
					<td class="ws-nowrap align-center">@if($data['credit'] != '') @format_currency($data['credit']) @endif</td>
					<td class="ws-nowrap align-center">{{$data['balance']}}
					<?php $rbalance=$data['balance']; ?></td>
					<!--<td>{{$data['payment_method']}}</td>-->
				
					<!--<td class="ws-nowrap align-center">-->
					<!--	{!! $data['others'] !!}-->
		...
		<td class="align-right">
		    {{$rbalance}}
		    <!--@format_currency($ledger_details['total_invoice']- $ledger_details['total_paid'])-->
		    </td>
