...
class PayrollController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            // Exclude admin users' payroll for subadmins
            $currentUser = auth()->user();
            $user_role_id = $currentUser->roleId ?? 0;
            if ($user_role_id == 14) {
                $util = new \App\Utils\Util();
                $admin_user_ids = $util->getAdminUserIds($business_id);
                $payrolls->whereNotIn('transactions.expense_for', $admin_user_ids);
            }
    ...
    {
        // Only include active users for payroll
        ...
        if ($user_role_id == 14) {
            ...
            $query->whereNotIn('id', $admin_user_ids);
        }

        // If a location is provided, filter by it. If not provided, return users across all locations.
