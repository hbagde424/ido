...
class ToDoController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($todos)
                ...
                ->addColumn('project_name', function ($row) {
                    if ($row->projectChecklist) {
                        return $row->projectChecklist->project_name;
                    }
                    return '-';
                })
    ...
    public function create()
    {
        ...
        if (auth()->user()->can('essentials.assign_todos')) {
            ...
            if ($user_role_id == 1) {
                ...
                // Other roles with assign_todos permission - Show users based on location permissions
                $admin_user_ids = $this->commonUtil->getAdminUserIds($business_id);
                
                if ($currentUser->can('access_all_locations')) {
                    // Can see all users except admins
                    ...
                    $users = $all_users->mapWithKeys(function ($user) {
                        $full_name = trim(($user->surname ?? '') . ' ' . ($user->first_name ?? '') . ' ' . ($user->last_name ?? ''));
                        return [$user->id => $full_name];
                    })->filter(function ($name) {
                        return !empty(trim($name));
                    ...
                    // Show users from permitted locations only
                    $permitted_locations = $currentUser->permitted_locations();
                    ...
                    if ($permitted_locations !== 'all' && !empty($permitted_locations)) {
                        ...
                        // No location permissions - show only themselves
                        $current_user = User::find($auth_id);
                        if ($current_user) {
                            $full_name = trim(($current_user->surname ?? '') . ' ' . ($current_user->first_name ?? '') . ' ' . ($current_user->last_name ?? ''));
                            $users = [$auth_id => $full_name];
        ...
        // Get projects for the business (explicitly exclude soft-deleted projects)
    ...
    public function addComment(Request $request)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                // Sync comment to Project Task if linked
                if ($todo->projectTask) {
                    \App\ProjectTaskComment::create([
                        'project_task_id' => $todo->projectTask->id,
                        'comment' => $input['comment'],
                        'user_id' => $auth_id,
                    ]);
                }

                // Load the added_by relationship for the comment view
    ...
    public function uploadDocument(Request $request)
    {
        ...
        try {
            ...
            // Sync documents to Project Task if linked
            if ($todo->projectTask) {
                // Get all media for this todo
                $todoMediaIds = $todo->media()->pluck('id')->toArray();
                
                // Get media IDs already synced to project task
                $projectTaskMediaFiles = \App\Media::where('model_type', 'App\ProjectTask')
                    ->where('model_id', $todo->projectTask->id)
                    ->pluck('file_name')
                    ->toArray();
                
                // Sync any new media that hasn't been synced yet
                $newMedia = $todo->media()
                    ->whereNotIn('file_name', $projectTaskMediaFiles)
                    ->get();
                
                foreach ($newMedia as $media) {
                    $replicatedMedia = $media->replicate();
                    $replicatedMedia->model_type = 'App\ProjectTask';
                    $replicatedMedia->model_id = $todo->projectTask->id;
    ...
    public function getTaskStatusChartData(Request $request)
    {
        ...
        // Initialize variables for subadmin filtering
        $admin_user_ids = [];
        $permitted_locations = [];
        ...
        if ($user_role_id == 1) {
            ...
            // Exclude admin users' data (variables already defined above)
            ...
            if (!empty($permitted_locations)) {
                ...
                $query->whereHas('users', function ($q) use ($permitted_locations, $admin_user_ids) {
                    ...
                    if (!empty($admin_user_ids)) {
                        $q->whereNotIn('user_id', $admin_user_ids);
                    }
                });
            } else {
                $query->whereHas('users', function ($q) use ($auth_id) {
        ...
        // Apply role-based filtering to employee query (same as main query)
        if ($user_role_id == 1) {
            // Admin role - See all TODOs
        } elseif ($user_role_id == 14) {
            // Sub-admin role - Filter by location permissions and exclude admin users
            // Variables already defined above
            if (!empty($admin_user_ids)) {
                $employee_query->whereNotIn('u.id', $admin_user_ids);
            }
            
            if (!empty($permitted_locations)) {
                $employee_query->whereIn('u.location_id', $permitted_locations);
            } else {
                $employee_query->where('tu.user_id', $auth_id);
            }
        } else {
            // Other roles - Show only their own TODOs
            $employee_query->where('tu.user_id', $auth_id);
    ...
    public function getCalendarData(Request $request)
    {
        ...
        foreach ($todos as $todo) {
            ...
            // Format start date (strip time component for all-day events)
            $start_date = null;
            if ($todo->date) {
                // Extract just the date part, ignoring time
                $date_str = is_string($todo->date) ? $todo->date : $todo->date->format('Y-m-d H:i:s');
                $start_date = date('Y-m-d', strtotime($date_str));
            }
            
            // Format end date - FullCalendar's end date is EXCLUSIVE (day after the event ends)
            // So if task ends on Sept 6, we need to set end to Sept 7
            $end_date = null;
            if ($todo->end_date) {
                // Extract just the date part, ignoring time
                $end_date_str = is_string($todo->end_date) ? $todo->end_date : $todo->end_date->format('Y-m-d H:i:s');
                $end_date_only = date('Y-m-d', strtotime($end_date_str));
                // Add one day to make it exclusive for FullCalendar
                $end_date = date('Y-m-d', strtotime($end_date_only . ' +1 day'));
            } elseif ($start_date) {
                // If no end date, set it to start + 1 day for single-day events
            ...
            $event = [
                ...
                'start' => $start_date,
                'end' => $end_date,
                'allDay' => true, // Mark as all-day event for proper alignment
                'color' => $color,
                ...
                'extendedProps' => [
                    ...
                    'start_date' => $todo->date ? date('Y-m-d', strtotime($todo->date)) : null,
                    'end_date' => $todo->end_date ? date('Y-m-d', strtotime($todo->end_date)) : null
                ]
            ];
