...
// Use global function if available, otherwise use this handler
if (typeof window.handleNewProjectClick === 'function') {
    $(document).on('click', 'button.add_new_project', function(e) {
        console.log('project.js: jQuery handler triggered, calling global function');
        return window.handleNewProjectClick(this, e);
    });
} else {
    $(document).on('click', 'button.add_new_project', function(e) {
        e.preventDefault();
        ...
        var $modal = $('#project_model');
        
        console.log('project.js: New Project button clicked. URL:', url);
        console.log('project.js: Modal element found:', $modal.length > 0);
        
        if (!url) {
            console.error('Project create URL not found');
            if (typeof toastr !== 'undefined') {
                toastr.error('Unable to load create project form. Please refresh the page.');
            } else {
                alert('Unable to load create project form. Please refresh the page.');
            }
            return false;
        }
        ...
        $.ajax({
            ...
            success: function(result){
                console.log('project.js: AJAX success. Result length:', result ? result.length : 0);
                ...
                if (result && result.trim().length > 0) {
                    ...
                    if (typeof toastr !== 'undefined') {
                        toastr.error('Unable to load create project form.');
                    } else {
                        alert('Unable to load create project form.');
                    }
                }
            },
            error: function(xhr, status, error) {
                console.error('project.js: Error loading project create form:', error, xhr);
                $button.prop('disabled', false);
                
                var errorMsg = 'Unable to load create project form.';
                if (xhr.status === 403) {
                    errorMsg = 'You do not have permission to create projects.';
                } else if (xhr.status === 404) {
                    errorMsg = 'Create project form not found.';
...
$('#project_model').on('shown.bs.modal', function (e) {
    ...
    setTimeout(function() {
        ...
        $('#project_model form#project_form .datepicker').each(function() {
        ...
        // Initialize select2 dropdowns within the modal
        $('#project_model form#project_form .select2').each(function() {
        ...
        if ($('#project_model textarea#description').length && typeof tinymce !== 'undefined') {
            tinymce.init({
                selector: '#project_model textarea#description',
            });
        }
        
        // Form validation with custom submit handler
        if ($('#project_model form#project_form').length) {
            $('#project_model form#project_form').validate({
                submitHandler: function(form, event) {
                    if (typeof window.handleProjectFormSubmit === 'function') {
                        return window.handleProjectFormSubmit(form, event);
                    }
                    return false;
                }
            });
...
$(document).on('submit', 'form#project_form', function(e){
    ...
    console.log('Project form submitting. URL:', url, 'Method:', method);
    
    // Get ladda button within the form/modal
    var laddaButton = $form.find('.ladda-button')[0] || document.querySelector('#project_model .ladda-button');
    var ladda = null;
    if (laddaButton) {
        ladda = Ladda.create(laddaButton);
        ladda.start();
    }
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            if (result && result.success) {
                ...
                if (typeof toastr !== 'undefined' && result.msg) {
                ...
                if (project_view == 'kanban') {
                    if (typeof initializeProjectKanbanBoard === 'function') {
                        initializeProjectKanbanBoard();
                    }
                } else if(project_view == 'list_view') {
                    // Reload the page or refresh the project list
                ...
                var errorMsg = result && result.msg ? result.msg : 'An error occurred while saving the project.';
                if (typeof toastr !== 'undefined') {
                    toastr.error(errorMsg);
                } else {
                    alert(errorMsg);
        ...
        error: function(xhr, status, error) {
            console.error('Form submit error:', error, xhr);
            if (ladda) {
                ladda.stop();
            }
            
            var errorMsg = 'An error occurred while saving the project.';
