...
class EssentialsUtil extends Util
{
    ...
    public function clockin($data, $essentials_settings)
    {
        ...
        //Check if already clocked in (pending clock-out)
        $pending_count = EssentialsAttendance::where('business_id', $data['business_id'])
        ...
        $today = \Carbon\Carbon::parse($clock_in_time)->format('Y-m-d');
        $today_count = EssentialsAttendance::where('business_id', $data['business_id'])
                                ->where('user_id', $data['user_id'])
        ...
        if ($today_count == 0) {
            ...
            $output = ['success' => false,
                'msg' => __('essentials::lang.attendance_already_marked_today'),
                'type' => 'clock_in',
            ];
        }
    ...
    public function clockout($data, $essentials_settings)
    {
        ...
        //Get today's clock in entry that hasn't been clocked out
        $today = \Carbon\Carbon::parse($data['clock_out_time'])->format('Y-m-d');
