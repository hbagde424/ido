...
class EssentialsLeaveTypeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $leave_types = EssentialsLeaveType::where('business_id', $business_id)
                        ->select(['leave_type', 'max_leave_count', 'max_leaves_per_month', 'id']);

            return Datatables::of($leave_types)
                ->editColumn('max_leaves_per_month', function($row) {
                    return $row->max_leaves_per_month ? $row->max_leaves_per_month : 'No limit';
                })
                ->addColumn(
                    'action',
    ...
    public function store(Request $request)
    {
        ...
        try {
            $input = $request->only(['leave_type', 'max_leave_count', 'leave_count_interval', 'max_leaves_per_month']);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $input = $request->only(['leave_type', 'max_leave_count',
                'leave_count_interval', 'max_leaves_per_month']);
