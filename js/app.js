$(document).ready(function() {
    ...
    $(document).on('change', '#has_no_sell_from, #cg_filter, #status_filter, #assigned_to, #location_id', function(){
        contact_table.ajax.reload();
    });
    ...
    expense_table = $('#expense_table').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
                 var newTitle = "Expenses-"; // Set your new title here
                 if(data.length !== 0){
            $('title').text(newTitle+data[0].location_name);
                 }
        },
