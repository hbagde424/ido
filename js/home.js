$(document).ready(function() {
    ...
    var stock_alert_table = $('#stock_alert_table').DataTable({
        ...
        ajax: {
            ...
            "data": function ( d ) {
                var newTitle = "Stock Alert List"; // Set your new title here
            $('title').text(newTitle);
...
function update_statistics(start, end) {
    ...
    if ($('#dashboard_location').length > 0) {
        ...
        let data = { start: start, end: end, location_id: location_id };

    if(location_id == '')
    {
           data = { start:   moment().format('YYYY-MM-DD'), end:   moment().format('YYYY-MM-DD'), location_id: location_id };
    }
   ;
    ...
    $('.net').html(loader);
     ...
     $('.today_sell').html(loader);
     
      $('.monthly_sell').html(loader);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            //sell details
             $('.today_sell').html(__currency_trans_from_en(data.today_sell, true));
             $('.stock_check').html(__currency_trans_from_en(data.closing_stock_by_sp, true));
             
             
              $('.monthly_sell').html(__currency_trans_from_en(data.monthly_sell, true));
              
    $('.net_profit').html(__currency_trans_from_en(data.data.net_profit, true));
    
    $('.gross_profit').html(__currency_trans_from_en(data.data.gross_profit, true));
