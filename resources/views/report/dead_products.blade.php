...
<section class="content">
    <div class="row no-print">
        <div class="col-md-12">
            @component('components.filters', ['title' => __('report.filters')])
              {!! Form::open(['url' => action([\App\Http\Controllers\ReportController::class, 'getDeadProducts']), 'method' => 'get' ]) !!}
                <div class="col-md-3">
    ...
    <table class="table table-bordered table-striped ajax_view" id="sale_report_table">
        ...
        <tbody>
            @foreach($products as $eachpro)
                  <tr> 
            <td>{{$eachpro->product}}</td>
            <td>{{$eachpro->unit}}</td>
            <td>{{$eachpro->sku}}</td>
            <td>{{$eachpro->total_unit_sold}}</td>
             </tr>
            @endforeach
 ...
 <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
 	<div class="row" >
                        @component('components.widget', ['class' => 'box-primary', 'title' => 'Dead Product'])
