...
<div class="col-md-12 no-print pos-header" style="background:##DFF5FF;">
      ...
      <div class="m-6 mt-5" style="display: flex;">
        <p >
          @if(empty($transaction->location_id))
            ...
            <div style="width: 50%;margin-bottom: 5px;height:30px ">
            ...
            @else
              ...
              <input type="hidden" id="select_location_id" name="select_location_id" value="<?php echo $default_location->id; ?>">
            @endif
          @endif

          @if(!empty($transaction->location_id))       
          <input type="hidden" id="select_location_id" name="select_location_id" value="{{$transaction->location->id}}">
   {{$transaction->location->name}} @endif
   <select class="form-control select2"  id="transactionSelect" style="width: 50%;margin-bottom: 5px;height:30px !important;!important;font-weight:700;">
            <option><srtong>{{ __('lang_v1.add_payment') }}</srtong></option>
            <?php foreach($customerlist as $eachcust){ ?>
            <option value="<?php echo $eachcust->id; ?>"><?php echo $eachcust->name; ?></option>
            <?php } ?>
        </select>
        </p>
      </div>
    ...
    <div class="col-md-6">
      ...
      <!--</button>-->
       @can('isAdmin')
       <button class="btn" onclick="window.location.href='https://gaurishankar369.in/shreeshiv/public/account/account'" style="background: #1572E8;
    color: white;width:160px;height:45px;border-radius: 15px;margin:2px;">
            <strong>Deposit</strong></button>
      <button class="btn" onclick="#" style="background: #1572E8;
    color: white;width:160px;height:45px;border-radius: 15px;margin:2px;">
      ...
      @endif
        ...
        @can('expense.add')
        <button id="add_expense" class="btn" title="{{ __('expense.add_expense') }}"   
           style="background: #1572E8;
    color: white;width:160px;height:45px;border-radius: 15px;margin:2px;">
            <strong> @lang('expense.add_expense')</strong>
        </button>
        <button class="btn" style="background: #1572E8;
    color: white;width:160px;height:45px;border-radius: 15px;margin:2px;" id="return_sale" title="@lang('lang_v1.sell_return')" data-toggle="popover" data-trigger="click" data-content='<div class="m-8"><input type="text" class="form-control" placeholder="@lang("sale.invoice_no")" id="send_for_sell_return_invoice_no"></div><div class="w-100 text-center"><button type="button" class="btn btn-danger" id="send_for_sell_return">@lang("lang_v1.send")</button></div>' data-html="true" data-placement="bottom">
            <strong>@lang('lang_v1.sell_return')</strong>
      </button>
      <button class="btn" onclick="#" style="background: #1572E8;
    color: white;width:160px;height:45px;border-radius: 15px;margin:2px;font-weight:700;">
            <strong>Custom</strong>
      </button>
      <button class="btn" onclick="#" style="background: #1572E8;
    color: white;width:160px;height:45px;border-radius: 15px;margin:2px;" >
