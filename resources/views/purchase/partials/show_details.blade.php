...
<div class="modal-body">
  ...
  <div class="row invoice-info">
    ...
    <div class="col-sm-4 invoice-col">
      ...
      <address>
        <strong style="display:none;">{{ $purchase->business->name }}</strong>
        {{ $purchase->location->name }}
        
        @if(!empty($purchase->location->city) || !empty($purchase->location->country))
          <br>{{implode(',', array_filter([$purchase->location->city, $purchase->location->country]))}}
        @endif
        ...
        <table class="table bg-gray">
          <thead>
            <tr class="bg-green">
              ...
              <th>@lang('product.product_image')</th>
              <th>@lang('product.sku')</th>
              @if($purchase->type == 'purchase_order')
                <th>@lang( 'lang_v1.quantity_remaining' )</th>
              @endif
              <th >@if($purchase->type == 'purchase_order') @lang('lang_v1.order_quantity') @else @lang('purchase.purchase_quantity') @endif</th>
              <th>Unit Cost</th>
              <th class="text-right" style="display:none;">@lang( 'lang_v1.discount_percent' )</th>
              <th class="no-print text-right" style="display:none;">@lang('purchase.unit_cost_before_tax')</th>
              <th class="no-print text-right" style="display:none;">@lang('purchase.subtotal_before_tax')</th>
              <th class="text-right" style="display:none;">@lang('sale.tax')</th>
              <th class="text-right" style="display:none;">@lang('purchase.unit_cost_after_tax')</th>
              @if($purchase->type != 'purchase_order')
              @if(session('business.enable_lot_number'))
                <th style="display:none;">@lang('lang_v1.lot_number')</th>
              @endif
              @if(session('business.enable_product_expiry'))
                <th style="display:none;">@lang('product.mfg_date')</th>
                <th style="display:none;">@lang('product.exp_date')</th>
              @endif
              @endif
          ...
          @foreach($purchase->purchase_lines as $purchase_line)
            <tr>
              ...
              <td>
            <img src="https://gaurishankar369.in/shreeshiv/public/uploads/img/<?php echo $purchase_line->product->image  ?>" alt="Product image" style="width:80px"> 
              ...
              @if($purchase->type == 'purchase_order')
              <td>
                <span class="display_currency" data-is_quantity="true" data-currency_symbol="false">{{ $purchase_line->quantity - $purchase_line->po_quantity_purchased }}
             ...
             <meta name="csrf-token" content="{{ csrf_token() }}">
              ...
              <?php if(($purchase_line->quantity - $purchase_line->po_quantity_purchased)>0){ ?> 
                ...
                <!--<form action={{route('updatepostatus')}} method="post" class="no-print form<?php echo $purchase_line->id ?>">-->
                    <input type='hidden' value="{{$purchase_line->id }}" name="id" />
                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">

                    <select id="postatus<?php echo $purchase_line->id ?>" name='postatus'>
                        <option value="">Select Status</option>
                        <option <?php if($purchase_line->postatus=='on the way'){ echo "selected";} ?> value="on the way">On the way</option>
                        <option <?php if($purchase_line->postatus=='ordered'){ echo "selected";} ?> value="ordered">ordered</option>
                    </select>
                <!--</form>-->
                    <?php } ?>
...
$(document).ready(function(){
   $('#postatus'+<?php echo $purchase_line->id ?>).change(function(){
        var inputValue = $(this).val();
         var csrfToken = $('meta[name="csrf-token"]').attr('content');
        // AJAX call to send the updated value to the server
        $.ajax({
            url: '{{ route("updatepostatus") }}', // Assuming you have a route named 'updateValue' defined in your routes file
            method: 'POST',
            data: {
                postatus: inputValue,
                  _token: csrfToken,
                  id:'<?php echo $purchase_line->id ?>'
            },
            ...
            error: function(xhr, status, error){
              ...
              @endif
                ...
                <!--{{$purchase_line->product->unit->short_name}} @endif-->
              </td>
              @endif
              <td><span class="display_currency" data-is_quantity="true" data-currency_symbol="false">{{ $purchase_line->quantity }}
              </span> @if(!empty($purchase_line->sub_unit)) {{$purchase_line->sub_unit->short_name}} @else
              {{$purchase_line->product->unit->short_name}} @endif

                @if(!empty($purchase_line->product->second_unit) && $purchase_line->secondary_unit_quantity != 0)
                    <br>
                    <span class="display_currency" data-is_quantity="true" data-currency_symbol="false">
                        {{ $purchase_line->secondary_unit_quantity }}</span> {{$purchase_line->product->second_unit->short_name}}
                @endif
              ...
              @if($purchase->type != 'purchase_order')
              @if(session('business.enable_lot_number'))
                <td style="display:none;">{{$purchase_line->lot_number}}</td>
              @endif
