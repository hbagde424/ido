...
class Media extends Model
{
    ...
    {
        // Check if file exists in public/uploads/media
        $publicPath = public_path('uploads/media/'.rawurlencode($this->file_name));
        $storagePath = storage_path('app/media/'.rawurlencode($this->file_name));
        
        if (file_exists($publicPath)) {
            $path = asset('uploads/media/'.rawurlencode($this->file_name));
        } elseif (file_exists($storagePath)) {
            // File in storage, use storage URL (requires symlink: php artisan storage:link)
            $path = url('storage/media/'.rawurlencode($this->file_name));
        } else {
    ...
    public static function uploadFile($file)
    {
        ...
        if ($file->getSize() <= config('constants.document_size_limit')) {
            ...
            // Store file using Storage facade with local disk
